...
// Rutas de la API
// 1. Obtener todos los clientes con paginación
app.get('/clientes', async (req, res) => {
    try {
        // Obtener los parámetros de consulta
        const page = parseInt(req.query.page) || 1;  // Número de página, por defecto es 1
        const limit = parseInt(req.query.limit) || 50;  // Número de clientes por página, por defecto es 50
        const offset = (page - 1) * limit;  // Calcular el desplazamiento (offset)

        // Ejecutar la consulta SQL con LIMIT y OFFSET para la paginación
        const result = await client.query(
            'SELECT * FROM Clientes LIMIT $1 OFFSET $2', 
            [limit, offset]
        );

        // Obtener el total de clientes para calcular el total de páginas
        const countResult = await client.query('SELECT COUNT(*) FROM Clientes');
        const totalClients = parseInt(countResult.rows[0].count);
        const totalPages = Math.ceil(totalClients / limit);  // Calcular el total de páginas
        ...
        res.json({
            ...
            totalPages,
            totalClients,
            clients: result.rows  // Devolver los clientes paginados
        });
    } catch (err) {
